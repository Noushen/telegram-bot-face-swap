...
def detect_face_points(img, face, predictor_path):
    predictor = dlib.shape_predictor(predictor_path)
    landmarks = predictor(img, face)
    landmarks_points = np.asarray(list([p.x, p.y] for p in landmarks.parts()), dtype=np.int)

    # # draw face dots
    # for dot in landmarks_points:
    #     cv2.circle(img, (dot[0], dot[1]), 1, (0, 0, 255), -1)

    return landmarks_points


def find_face_shape(img, points, r=10):
...
def find_face_points_shape_crop(img, predictor_path):
    face = find_face(img)
    points = detect_face_points(img, face, predictor_path)
    shape = find_face_shape(img, points)
    crop = crop_img(img, shape)
    shift_points = points - np.asarray([[shape[0], shape[1]]])
    return shift_points, shape, crop
...
def face_swap(src_face, dst_face, src_points, dst_points, dst_shape, dst_img, correct_color=True, warp_2d=False, end=68):
...
def pretty_face_swap(src_img_path, dst_img_path, predictor_path):
    src_img = cv2.imread(src_img_path)
    dst_img = cv2.imread(dst_img_path)

    src_points, src_shape, src_face = find_face_points_shape_crop(src_img, predictor_path)
    dst_points, dst_shape, dst_face = find_face_points_shape_crop(dst_img, predictor_path)

    if src_points is None or dst_points is None:
        print('Detect 0 Face !!!')

    result_img = face_swap(src_face, dst_face, src_points, dst_points, dst_shape, dst_img)
    return result_img
